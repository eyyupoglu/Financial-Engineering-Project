...
gmv_Ret= AvAnRet * weight_GMV; %Same Standard deviation as the previous GMV
...
MaxRet_Port_Ret=max(AvAnRet);
MaxRet_Port_StDv=StDvAn(Id_Max_Ret);
% Equal weights portfolio
weight_equal=ones(12,1)/12;
Ret_equal=AvAnRet*weight_equal;
StDv_equal=sqrt(weight_equal'*AnCov*weight_equal);
...
xlabel('Risk');
ylabel('Return');
hold on %We remove the dominated stocks MSFT, EBAY, VMC, JPM, F, GE, XOM, WMT and K.
...
new_StDv_GMV = sqrt(1 / (new_I'/new_AnCov*new_I));
...
new_StDv_equal = sqrt(new_weight_equal' * new_AnCov * new_weight_equal);
